    ...
    $hdrBtmGubLi.forEach((lis, idx) => {
        lis.addEventListener('mouseenter', (e) => {
            ...
            if (current !== old) {
                ...
                $hdrBtmBg.style.height = `${$hdrBtmSubwrap[current].offsetHeight + 50}px`;
                old = current;
            } else {
                $hdrBtmGubLi[current].classList.add('active');
                $hdrBtmBg.style.height = `${$hdrBtmSubwrap[current].offsetHeight + 50}px`;
...
const familySite = () => {
    ...
    let $subWrap = getAll('#footer .footer-top-box .top-box-inner > ul > li > .sub-wrap');
    let $footerTopBoxAbtn = getAll('#footer .footer-top-box .top-box-inner > ul > li > a');
    ...
    $footerTopBoxAbtn.forEach((aBtn, idx) => {
        aBtn.addEventListener('click', (e) => {
            ...
            if (current !== old) {
                ...
                $footerTopBoxUl.style.height = `${$subWrap[current].offsetHeight + 55}px`;
                old = current;
            } else {
